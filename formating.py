...
def formating(file,customer_id,now):
    
    wb = xlrd.open_workbook(file, logfile=open(os.devnull, 'w')) # remove WARNINGS
    df = pd.read_excel(wb, skiprows=10, usecols="A:C,E,F,I,J,M:S,U", engine='xlrd')
    ...
    cell_format_order = workbook.add_format({'bg_color': '#FFFFBE', 'border': 1})
    ...
    row_numbers = df.shape[0] + 6
    worksheet.write_dynamic_array_formula('O2', f'=SUM($I$7:$I${row_numbers}*$O$7:$O${row_numbers})',cell_format) # Total weight
    worksheet.write_dynamic_array_formula('O3', f'=SUM($J$7:$J${row_numbers}*$O$7:$O${row_numbers})',cbm_format) # Total CBM
    worksheet.write_dynamic_array_formula('O4', f'=SUM($K$7:$K${row_numbers}*$O$7:$O${row_numbers})',cell_format_) # Total Cartoons
    ...
    worksheet.write_dynamic_array_formula('P3', f'=SUM($J$7:$J${row_numbers}*$P$7:$P${row_numbers})',cbm_format) # Total CBM
    worksheet.write_dynamic_array_formula('P4', f'=SUM($K$7:$K${row_numbers}*$P$7:$P${row_numbers})',cell_format_) # Total Cartoons
    ...
    worksheet.conditional_format(f'A7:Q{row_numbers}', {'type': 'no_errors', 'format': cell_format_})
    worksheet.conditional_format(f'P7:P{row_numbers}', {'type': 'no_errors', 'format': cell_format_order})
